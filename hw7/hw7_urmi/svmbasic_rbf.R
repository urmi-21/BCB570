...
# train the SVM
svp <- ksvm(xtrain,ytrain,type="C-svc",kernel='rbfdot',C=1,scaled=c())
...
k=5	
ypredscorecv <- cvpred.ksvm(x,y,folds=k,type="C-svc",kernel='rbfdot',C=1,scaled=c(),predtype="decision")
...
for (i in seq(length(clist))) {
	svp <- ksvm(x,y,type="C-svc",kernel='rbfdot',C=clist[i],scaled=c(),cross=3)
...
# Train a linear SVM
ypred <- cvpred.ksvm(x,y,type="C-svc",kernel='rbfdot',C=1)
print(table(ypred,y))
...
for (i in seq(length(clist))) {
	cat('.')
	svp <- ksvm(x,y,type="C-svc",kernel='rbfdot',C=clist[i],cross=3)
	errlin[i] <- cross(svp)
	svp <- ksvm(x,y,type="C-svc",kernel='rbf',C=clist[i],cross=3)
	errrbf[i] <- cross(svp)
}
